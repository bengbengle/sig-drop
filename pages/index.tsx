...
const signatureDropAddress = "0xDAd130cD4F1b6E5ea3cf01FDe61c3A0ae24161a1";

const Home: NextPage = () => {
  const address = useAddress();

  const { contract: signatureDrop } = useContract(signatureDropAddress, "signature-drop");
  ...
  async function claimWithSig() {
    const sig = await fetch("/api/signature", {
      method: "POST",
      body: JSON.stringify({
        address: address,
      }),
    ...
    if (sig.status === 400) {
      ...
      try {
        const signedPayload = (await sig.json()) as SignedPayload721WithQuantitySignature;
